    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-slate-500">{classes.length} classes total</p>
        </div>
      ...
      {classes.length === 0 ? (
        <div className="card">
          <EmptyState icon={BookOpen} title="No classes yet" description="Create the first class to get started" />
        </div>
      ) : (
          ...
          {classes.map((c) => {
            ...
            return (
              <div key={c._id} className="card hover:shadow-md transition-shadow">
                ...
                <h3 className="font-bold text-slate-800 text-lg mb-1">{c.className}</h3>
                {c.teacher ? (
                  <p className="text-sm text-slate-500 mb-3">
                    Teacher: <span className="font-medium text-slate-700">{c.teacher?.userId?.fullName}</span>
                  </p>
                ) : (
                  <p className="text-sm text-yellow-600 mb-3">No teacher assigned</p>
                )}
                ...
                <div className="mb-3">
                  <div className="flex justify-between text-xs text-slate-500 mb-1">
                    ...
                    <span>{c.currentStudents}/{c.maxStudents}</span>
                  </div>
                ...
                <button
                  onClick={() => { setAssignOpen(c); setSelectedTeacher('') }}
      ...
      {/* Create Modal */}
      <Modal isOpen={createOpen} onClose={() => setCreateOpen(false)} title="Create New Class" size="sm">
      ...
      <Modal isOpen={!!assignOpen} onClose={() => setAssignOpen(null)} title="Assign Teacher" size="sm">
        <div className="space-y-4">
          ...
          <div>
            ...
            <select className="input" value={selectedTeacher} onChange={(e) => setSelectedTeacher(e.target.value)}>
              ...
              {teachers
                .filter((t) => {
                  if (assignOpen?.classType === 'FRESH') return t.teacherType === 'FRESH_TEACHER'
                  if (assignOpen?.classType === 'ADVANCED') return t.teacherType === 'ADVANCED_TEACHER'
                  return true
                })
                .map((t) => (
                  <option key={t._id} value={t._id}>{t.userId?.fullName} ({t.teacherType})</option>
                ))}
            </select>
          </div>
          <div className="flex gap-3">
            <button onClick={handleAssignTeacher} className="btn-primary flex-1" disabled={submitting}>
              {submitting ? 'Assigning...' : 'Assign'}
            </button>
            <button onClick={() => setAssignOpen(null)} className="btn-secondary flex-1">Cancel</button>
          </div>
        </div>
      </Modal>
    </div>
