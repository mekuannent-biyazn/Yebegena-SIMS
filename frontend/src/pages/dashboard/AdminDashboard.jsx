  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
        ...
        <StatCard
          icon={UserPlus}
          label="Fresh Students"
          value={s.freshStudents}
          color="indigo"
        />
        <StatCard
          icon={TrendingUp}
          label="Advanced Students"
          value={s.advancedStudents}
          color="purple"
        />
        <StatCard
          icon={GraduationCap}
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
        <StatCard
          ...
          value={s.upcomingExams}
          color="indigo"
        />
        <StatCard
          icon={RefreshCw}
          label="Class Change Requests"
          value={s.classChanges}
          color="yellow"
        />
        <StatCard
          icon={BookOpen}
          ...
          {stats?.charts?.paymentOverview?.length > 0 ? (
            <div className="space-y-3">
              {stats.charts.paymentOverview.map((item) => {
                const total = stats.charts.paymentOverview.reduce(
                  (a, b) => a + b.total,
                  0,
                );
                const pct =
          ...
          {stats?.charts?.monthlyRegistrations?.length > 0 ? (
            <div className="space-y-2">
              {stats.charts.monthlyRegistrations.slice(-6).map((item) => {
                const max = Math.max(
                  ...stats.charts.monthlyRegistrations.map((r) => r.total),
                );
